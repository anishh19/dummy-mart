...
function MainSlider() {
  ...
  return (
    <div className="grow w-full relative" >
        <button id="prevSlide" onClick={()=>prevSlide()} className="absolute h-1/3 top-2 left-0 z-10 hover:border-2 rounded">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="White" className="w-10 h-10">
          <path fillRule="evenodd" d="M7.72 12.53a.75.75 0 010-1.06l7.5-7.5a.75.75 0 111.06 1.06L9.31 12l6.97 6.97a.75.75 0 11-1.06 1.06l-7.5-7.5z" clipRule="evenodd" />
          </svg>
        </button>
        <button id="nextSlide"  onClick={()=>nextSlide()} className="absolute h-1/3 top-2 right-0 z-10 hover:border-2 rounded">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white" className="w-10 h-10">
          <path fillRule="evenodd" d="M16.28 11.47a.75.75 0 010 1.06l-7.5 7.5a.75.75 0 01-1.06-1.06L14.69 12 7.72 5.03a.75.75 0 011.06-1.06l7.5 7.5z" clipRule="evenodd" />
          </svg>
        </button>
        <div id="slide-1" className="h-full w-full absolute slide ">
            <div className="bg-pink-800 h-1/3 w-full">
            </div>
